...
from aiq_aira.functions.generate_summary import AIRAGenerateSummaryConfig
from aiq_aira.functions.generate_summary import GenerateSummaryStateInput
...
generate_summary_user_post_input = GenerateSummaryStateInput(
    ...
    report_organization=
    "Can you create a detailed report on NVIDIA's 2025 earnings that covers its key financial numbers, market and competition factors, tech investments, global economic and political trends, and future outlook using the provided docs and data visuals?",
    queries=[
        GeneratedQuery(query="nvidia earnings", report_section="main body", rationale="its important"),
        GeneratedQuery(query="nvidia competitors", report_section="conclusion", rationale="other information")
    ],
    reflection_count=1,
    search_web=False,
...
aira_state = AIRAState(queries=generate_summary_user_post_input.queries)
...
async def mock_rag_relevant(aiohttp_client):
    ...
    async def handler(request):
        ...
        assert data[
            "collection_name"] == "user_passed_collection", f"Got collection: {data['collection_name']} but expected user_passed_collection"
...
async def test_web_research_relevant(mock_rag_relevant, reasoning_llm, capsys):
    ...
    aira_config = AIRAGenerateSummaryConfig(rag_url=url)
    ...
    async with ClientSession() as session:
        ...
        result = await web_research(aira_state, {"configurable": user_input_passed_as_langchain_config}, print)
