...
SAMPLE_QUERIES = [
    GeneratedQuery(
        ...
        rationale=
        "Provides an overview of Amazon's 2023 financial highlights and business segments for contextualizing the report."
    ),
    GeneratedQuery(
    ...
    GeneratedQuery(query="List of big mac ingredients",
...
async def test_generate_summary_basic(workflow_builder):
    ...
    async for builder in workflow_builder:
        ...
        assert any("summarize_sources" in r.intermediate_step.lower() for r in intermediate_results
                   if r.intermediate_step)
        assert any("reflect_on_summary" in r.intermediate_step.lower() for r in intermediate_results
                   if r.intermediate_step)
        assert any("final_report" in r.intermediate_step.lower() for r in intermediate_results if r.intermediate_step)
        assert any("relevancy_checker" in r.intermediate_step.lower() for r in intermediate_results
                   if r.intermediate_step)
...
async def test_generate_summary_no_web(workflow_builder):
    ...
    async for builder in workflow_builder:
        ...
        input_data = GenerateSummaryStateInput(topic="Renewable Energy Technologies",
