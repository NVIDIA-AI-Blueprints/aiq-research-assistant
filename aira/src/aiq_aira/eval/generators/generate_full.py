...
from aiq_aira.eval.generator_register import AIRAGeneratorBase, register_generator
from .extraction_utils import (
    extract_rag_contexts,
    extract_relevancy_judgements,
    extract_web_answers,
    extract_reflections_and_queries,
    split_report_and_citations,
    extract_groundness_facts,
    pair_facts_with_citations,
    generate_context_relevance_questions,
    generate_coverage_facts_claims
)
from aiq_aira.schema import GenerateQueryStateInput, GenerateSummaryStateInput
...
class AIRAFullGenerator(AIRAGeneratorBase):
    ...
    async def pre_process(self, eval_input: AIResearcherEvalInput) -> AIResearcherEvalInput:
        ...
        if not eval_input.context_relevance_questions or len(eval_input.context_relevance_questions) == 0:
            ...
            questions = await generate_context_relevance_questions(
                eval_input.topic, 
        ...
        if not eval_input.coverage_facts_claims or len(eval_input.coverage_facts_claims) == 0:
            ...
            facts = await generate_coverage_facts_claims(
                eval_input.ground_truth,
                llm=self.fact_extraction_llm,
                verbose=verbose
            )
    ...
    async def generate_queries_with_logging(self, input_data: GenerateQueryStateInput) -> Dict[str, Any]:
        ...
        return {
            "queries": final_output.queries if final_output else [],
        ...
        async for output in self.generate_summary_fn.astream(input_data):
            if hasattr(output, 'intermediate_step') and output.intermediate_step:
                ...
                if verbose:
                    ...
                    logger.debug("Summary Intermediate Step: %s", output.intermediate_step[:200] + "..." if len(output.intermediate_step) > 200 else output.intermediate_step)
            else:
                final_output = output
                if verbose:
                    logger.info("Final Report Generated!")
                    logger.info("  - Report length: %d characters", len(output.final_report) if output.final_report else 0)
    ...
    async def extract_evaluation_outputs(
        self, 
        eval_input: AIResearcherEvalInput,
        query_result: Dict[str, Any],
        summary_result: Dict[str, Any]
    ) -> AIResearcherEvalOutput:
        ...
        reflections, queries_from_reflections = extract_reflections_and_queries(
            summary_result["intermediate_steps"]
        )
        if verbose:
        ...
        finalized_summary, citation_section = split_report_and_citations(
            summary_result["final_report"]
        )
        if verbose:
        ...
        groundness_facts_claims = await extract_groundness_facts(finalized_summary, llm=self.fact_extraction_llm, verbose=verbose)
        if verbose:
