...
from aiq.eval.evaluator.evaluator_model import (
    EvalInput,
    EvalInputItem,
    EvalOutput,
    EvalOutputItem,
...
class RagasWrapperEvaluator:
    ...
    async def evaluate_item(self, item: EvalInputItem) -> EvalOutputItem:
        ...
        try:
            ...
            logger.info(f"Item output_obj length: {len(item.output_obj) if isinstance(item.output_obj, str) else 'N/A'}")
            ...
            logger.info(f"Item {data_source.id}: Found {len(rag_contexts)} RAG contexts and {len(web_answers)} web answers")
            ...
            logger.info(f"Item {data_source.id}: Response length: {len(response)}, Ground truth length: {len(ground_truth)}")
            ...
            if self.metric_name == "ContextRelevance":
                ...
                try:
                    ...
                    logger.error(f"Item {data_source.id}: Error calling RAGAs {self.metric_name}: {str(e)}", exc_info=True)
                    score = float('nan')
                
                # Handle NaN scores
                if math.isnan(score):
                    logger.warning(f"Item {data_source.id}: RAGAs {self.metric_name} returned NaN score, setting to 0.0")
                    score = 0.0
    ...
    async def evaluate(self, eval_input: EvalInput) -> EvalOutput:
        ...
        """
        logger.info(f"Starting batch evaluation with {len(eval_input.eval_input_items)} items using metric {self.metric_name}")
        ...
        logger.info(f"  - Score distribution: min={min(scores) if scores else 'N/A'}, max={max(scores) if scores else 'N/A'}")
