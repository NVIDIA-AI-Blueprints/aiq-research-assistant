...
class CitationQualityEvaluator:
    ...
    async def evaluate_item_all_metrics(self, item: EvalInputItem) -> List[EvalOutputItem]:
        ...
        # Precision
        results.append(EvalOutputItem(
            id=f"{item.id}_precision",
            score=precision,
            reasoning={**base_reasoning, "metric_type": "precision", "precision": precision}
        ))
        
        # Recall  
        results.append(EvalOutputItem(
            id=f"{item.id}_recall", 
            score=recall,
            reasoning={**base_reasoning, "metric_type": "recall", "recall": recall}
        ))
        ...
        results.append(EvalOutputItem(
            id=f"{item.id}_f1",
            score=f1_score, 
            reasoning={**base_reasoning, "metric_type": "f1_score", "f1_score": f1_score}
        ))
    ...
    async def evaluate(self, eval_input: EvalInput) -> EvalOutput:
        ...
        precision_scores = [item.score for item in eval_output_items if item.reasoning.get("metric_type") == "precision"]
        ...
        logger.info(f"  - Precision Average: {sum(precision_scores) / len(precision_scores) if precision_scores else 0.0:.3f}")
...
class CitationPrecisionEvaluator:
    ...
    async def evaluate(self, eval_input: EvalInput) -> EvalOutput:
        ...
        precision_items = [
            item for item in base_result.eval_output_items 
            if item.reasoning.get("metric_type") == "precision"
        ]
...
class CitationRecallEvaluator:
    ...
    async def evaluate(self, eval_input: EvalInput) -> EvalOutput:
        ...
        recall_items = [
            item for item in base_result.eval_output_items 
            if item.reasoning.get("metric_type") == "recall"
        ]
...
class CitationF1Evaluator:
    ...
    async def evaluate(self, eval_input: EvalInput) -> EvalOutput:
        ...
        f1_items = [
            item for item in base_result.eval_output_items 
            if item.reasoning.get("metric_type") == "f1_score"
        ]
