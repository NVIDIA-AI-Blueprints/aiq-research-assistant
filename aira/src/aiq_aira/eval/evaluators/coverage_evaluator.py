        ...
        for retry_attempt in range(self.retry):
            try:
                ...
                if retry_attempt < self.retry - 1:
                    ...
                    logger.error(f"Failed to get/parse LLM response for CoverageMetric after {self.retry} attempts: {str(e)}")
                    return 0.0
        return 0.0
...
class CoverageEvaluator:
    ...
    def __init__(self, llm: BaseLanguageModel, max_concurrency: int = 4, output_dir: str = None, extraction_model: str = None, extraction_provider: str = None, extraction_max_retries: int = None, extraction_temperature: float = None, extraction_max_tokens: int = None):
    ...
    async def evaluate_item(self, item: EvalInputItem) -> EvalOutputItem:
        ...
        if not ground_truth or not ground_truth.strip():
            return EvalOutputItem(
                ...
                reasoning={
                    ...
                    "debug_info": {
                        "has_ground_truth": data_source.ground_truth is not None,
                        "keys_in_item": list(data_source.model_dump().keys()) if isinstance(data_source, dict) else None,
                        "item_id": item.id,
                    }
                }
        ...
        try:
            ...
            scores = await asyncio.gather(
                *[evaluate_single_claim(claim) for claim in facts_claims]
            )
            ...
            return EvalOutputItem(id=data_source.id, score=0.0, reasoning={"error": f"Coverage evaluation failed: {str(e)}"})
